...
class EthStreamerAdapter:
    ...
    def export_all(self, start_block, end_block):
        ...
        # Export contracts
        contract_address = self.contract_filter.get_temp_set()
        contracts = []
        if self._should_export(EntityType.CONTRACT):
            # contracts = self._export_contracts(traces)
            contracts = self._extract_contracts(contract_address)
        # Export tokens
        tokens = []
    ...
    def _extract_contracts(self, contract_addresses):
        exporter = InMemoryItemExporter(item_types=['contract'])
